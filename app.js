...
episodes.forEach((episode) => {
  ...
  const episodeList = `
          <label for="${id}">
            <input type="checkbox" name="episode-${id}" id="${id}" onchange="checkboxClicked(event)"/>
            <span>${id} || ${name}</span>
      </label>`;

  li.innerHTML = episodeList;
  episodesContainer.appendChild(li);
});

let key = false;
document.addEventListener("keydown", (event) => {
  key = event.key === "Shift" ? true : false;
});

function checkboxClicked(event) {
  if (key && event.srcElement.checked) {
    bulkSelection(event.target.id);
  }
}

function bulkSelection(id) {
